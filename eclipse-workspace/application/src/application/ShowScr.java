...
public class ShowScr {
	...
	public ShowScr(String s,boolean i,boolean b)
	{
		gen=s;
		imdb=i;
		box=b;
	...
	protected void createContents() {
		...
		label_10.setText("");
		label_10.setBounds(10, 478, 1029, 18);
		...
		btnNewButton.setBounds(513, 538, 118, 39);
		btnNewButton.setText("Back");
		String qry="select id,title,genre,year,imdb,box_office from film natural join screenwriter where name='"+gen+"' order by ";
		if(imdb)
			qry+="imdb";
		else
			qry+="box_office";
		...
		try {
			...
			ResultSet rs1=stmt.executeQuery("select sc_id from screenwriter where name='"+gen+"'");
			String idd="";
			if(rs1.next())
				idd=rs1.getString(1);
			...
			ResultSet rs3=stmt.executeQuery("select avg_box,avg_imdb from screenwriter where sc_id='"+idd+"'");
			if(rs3.next())
