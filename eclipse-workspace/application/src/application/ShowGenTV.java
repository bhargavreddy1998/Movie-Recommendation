...
public class ShowGenTV {
	...
	public ShowGenTV(String s,boolean i,boolean b)
	{
		gen=s;
		imdb=i;
		box=b;
	...
	public static void main(String[] args) {
		try {
			ShowGenTV window = new ShowGenTV();
			window.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void createContents() {
		...
		String qry="select id,title,genre,s_year,e_year,imdb,nielsen from series where genre='"+gen+"' order by ";
		if(imdb)
			qry+="imdb";
		else
			qry+="nielsen";
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("select id,title,genre,s_year,e_year,imdb,nielsen from series where genre='"+gen+"'");
			ndx = 0;
			while(rs.next())
			{
