...
public class ShowWriterTV {
	...
	public ShowWriterTV(String s,boolean i,boolean b)
	{
		gen=s;
		imdb=i;
		box=b;
	...
	public static void main(String[] args) {
		try {
			ShowWriterTV window = new ShowWriterTV();
			window.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void createContents() {
		...
		String idd="select wr_id from writer where name='"+gen+"'";
		try {
			Class.forName("oracle.jdbc.driver.OracleDriver");
			Connection con = DriverManager.getConnection("jdbc:oracle:thin:@localhost:1521:xe", "harold", "dashwood");
			Statement stmt = con.createStatement();
			ResultSet rs1 = stmt.executeQuery(idd);
			if(rs1.next())
			idd=rs1.getString(1);
			String qry="select id,title,genre,s_year,e_year,imdb,nielsen from series where wr_id='"+idd+"' order by ";
			if(imdb)
				qry+="imdb";
			else
				qry+="nielsen";
...
//			System.out.println(val);
	...
	public void stats(Label label_10) {
		...
		try {
			...
			ResultSet rs1 = stmt.executeQuery("select cas_id from tv_cast where lead_='" + gen + "'");
			String idd = "";
			if (rs1.next())
				idd = rs1.getString(1);

			ResultSet rs2 = stmt.executeQuery("select emmys,golden_globes from tv_awards where id='" + idd + "'");
			if (rs2.next())
				st += rs2.getString(1) + "		Golden Globes:" + rs2.getString(2);
			else
				st += "-NA-		Golden Globes:-NA-   ";
			st += "   Avg IMDB:";
			ResultSet rs3 = stmt.executeQuery("select avg_imdb,avg_nielsen from writer where wr_id='" + idd + "'");
			if (rs3.next())
				st += rs3.getString(1)+"    Avg Nielsen:"+rs3.getString(2);
			label_10.setText(st);
			con.close();
		} catch (Exception e) {
