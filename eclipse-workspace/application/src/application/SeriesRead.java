...
public class SeriesRead {
	...
	public static void main(String[] args) {
		try {
			SeriesRead window = new SeriesRead();
			window.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void createContents() {
		...
		ScreenwriterLabel.setText("Writer");
		new Label(shlQueryParameters, SWT.NONE);
		new Label(shlQueryParameters, SWT.NONE);
		...
		btnRadioButton_1.setText("Nielsen");
		new Label(shlQueryParameters, SWT.NONE);
		new Label(shlQueryParameters, SWT.NONE);
		new Label(shlQueryParameters, SWT.NONE);
		new Label(shlQueryParameters, SWT.NONE);
		...
		submitButton.addMouseListener(new MouseAdapter() {
			...
			public void mouseUp(MouseEvent e) {
				...
				if (isNotNull(id)||isNotNull(title))
					new ShowIdTitleTV(id,title);
				else if (isNotNull(genre))
					new ShowGenTV(genre,imdb,box);
				else if(isNotNull(dir))
					new ShowDirTV(dir,imdb,box);
				else if(isNotNull(scr))
					new ShowWriterTV(scr,imdb,box);
				else if(isNotNull(actor))
					new ShowActTV(actor,imdb,box);
