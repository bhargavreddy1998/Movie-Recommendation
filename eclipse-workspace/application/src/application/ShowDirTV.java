...
public class ShowDirTV {
	...
	public ShowDirTV(String s,boolean i,boolean b)
	{
		gen=s;
		imdb=i;
		box=b;
	...
	public static void main(String[] args) {
		try {
			ShowDirTV window = new ShowDirTV();
			window.open();
		} catch (Exception e) {
			e.printStackTrace();
		}
	...
	protected void createContents() {
		...
		String idd="select net_id from network where name='"+gen+"'";
		try {
			...
			if(rs1.next())
			idd=rs1.getString(1);
			String qry="select id,title,genre,s_year,e_year,imdb,nielsen from series where net_id='"+idd+"' order by ";
			if(imdb)
				qry+="imdb";
			else
				qry+="nielsen";
	...
	public void stats(Label label_10) {
		...
		try {
			...
			ResultSet rs1 = stmt.executeQuery("select net_id from network where name='" + gen + "'");
			String idd = "";
			if (rs1.next())
				idd = rs1.getString(1);
			...
			st += "   Budget:";
			ResultSet rs3 = stmt.executeQuery("select budget from network where net_id='" + idd + "'");
			if (rs3.next())
				st += rs3.getString(1);
			label_10.setText(st);
			con.close();
